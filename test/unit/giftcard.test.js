...
!developmentChains.includes(network.name)
    ...
    : describe("GiftCard NFT Unit Tests", async function () {
          let giftCard, mockV3Aggregator, tokenId
          ...
          before(async function () {
              ...
              mockV3Aggregator = await mockV3AggregatorFactory.deploy(DECIMALS, INITIAL_PRICE)

              const giftCardNFTFactory = await ethers.getContractFactory("GiftCard")
              giftCard = await giftCardNFTFactory.connect(deployer).deploy(mockV3Aggregator.address)
